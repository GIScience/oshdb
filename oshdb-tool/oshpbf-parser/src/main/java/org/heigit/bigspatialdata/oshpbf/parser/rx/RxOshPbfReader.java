...
public class RxOshPbfReader {
	...
	private static final byte[] SIGNATURE_OSMDATA = { /* wire_type */10, /* stringSize */7, 79, 83, 77, 68, 97, 116,
			97 };
	...
	public static Flowable<Osh> readOsh(Path pbfPath, long pos, long softLimit, long hardLimit,
		...
		}).filter(PbfBlob::isData).observeOn(Schedulers.computation(), false, prefetch)
				.map(blob -> blobToOSHItr(blob, false)).flatMap(Flowable::fromIterable);

...
public class TransformMapper2 {
  ...
  protected void initKeyTables(Connection connKeyTables) throws SQLException {
    pstmtKeyValue = connKeyTables.prepareStatement("" + //
        ...
        "from " + TableNames.E_KEY.toString() + " join " + TableNames.E_KEYVALUE.toString() + " on "
        + TableNames.E_KEY.toString() + ".id = " + TableNames.E_KEYVALUE.toString() + ".keyid " + //
        "where " + TableNames.E_KEY.toString() + ".txt = ? and " + TableNames.E_KEYVALUE.toString()
        + ".txt = ? ");
  ...
  protected int[] getKeyValue(List<OSMPbfTag> tags) {
    ...
    Collections.sort(sortedKeyValues, (a, b) -> {

...
public abstract class OSHDbHandler extends LoaderHandler {
  ...
  public void handleNodeGrid(long zId, Collection<TransformOSHNode> nodes) {
    ...
    try {
      GridOSHNodes grid = GridOSHNodes.rebase(xyId, zoom, gridNodes.get(0).getId(), 0, longitude, latitude, gridNodes);
      handleNodeGrid(grid);
  ...
  public void handleWayGrid(long zId, Collection<TransformOSHWay> ways, Collection<TransformOSHNode> nodes) {
    ...
    try {
      GridOSHWays grid = GridOSHWays.compact(xyId, zoom, gridWays.get(0).getId(), 0, longitude, latitude, gridWays);
      handleWayGrid(grid);
    } catch (IOException e) {
      throw new RuntimeException(e);
    }
  ...
  public void handleRelationGrid(long zId, Collection<TransfomRelation> entities, Collection<TransformOSHNode> nodes,
    ...
    try {
      GridOSHRelations grid = GridOSHRelations.compact(xyId, zoom, gridRelation.get(0).getId(), 0, longitude, latitude, gridRelation);

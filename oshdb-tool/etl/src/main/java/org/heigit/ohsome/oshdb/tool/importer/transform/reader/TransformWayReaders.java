...
public class TransformWayReaders implements Closeable {
  private static class TransformWayReader extends TransformReader<TransformOSHWay> {
    ...
    @Override
    protected TransformOSHWay getInstance(byte[] data, int offset, int length, long baseId,
        long baseTimestamp, long baseLongitude, long baseLatitude) throws IOException {
      return TransformOSHWay.instance(data, offset, length, baseId, baseTimestamp, baseLongitude,
          baseLatitude);
    }
  }

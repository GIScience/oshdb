...
public class TagTranslator {
   ...
   * versa.
   *
   * @param osmRole new OSMRole
   * @param oshdbRole new OSHDBRole
   */
  public void updateRole(OSMRole osmRole, OSHDBRole oshdbRole) {
    this.roleToInt.put(osmRole, oshdbRole);
    this.roleToString.put(oshdbRole, osmRole);
  }

  /**
   * Override the internal caches mapping for the given OSMTag with the new OSHDBTag and vice versa.
   *
   * @param osmTag new OSMTag
   * @param oshdbTag new OSHDBTag
   */
  public void updateTag(OSMTag osmTag, OSHDBTag oshdbTag) {
    this.keyToInt.put(new OSMTagKey(osmTag.getKey()), new OSHDBTagKey(oshdbTag.getKey()));
    this.keyToString.put(new OSHDBTagKey(oshdbTag.getKey()), new OSMTagKey(osmTag.getKey()));
    this.tagToInt.put(osmTag, oshdbTag);
    this.tagToString.put(oshdbTag, osmTag);
  }

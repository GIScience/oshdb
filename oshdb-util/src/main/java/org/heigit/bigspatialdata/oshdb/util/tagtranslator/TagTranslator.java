...
public class TagTranslator {
  ...
  public Pair<Integer, Integer> tag2Int(Pair<String, String> tag) {
    ...
    if (this.tagToInt.containsKey(keyString)
        && this.tagToInt.get(keyString).getValue().containsKey(valueString)) {
      return new ImmutablePair<>(this.tagToInt.get(keyString).getKey(),
          this.tagToInt.get(keyString).getValue().get(valueString));
    }

    // key or value is not in cache so let's go get them
    try (PreparedStatement valstmt =
        this.conn.prepareStatement("select k.ID as KEYID,kv.VALUEID as VALUEID from "
            + TableNames.E_KEYVALUE.toString() + " kv inner join " + TableNames.E_KEY.toString()
            + " k on k.ID = kv.KEYID WHERE k.TXT = ? and kv.TXT = ?;")) {
  ...
  public Pair<String, String> tag2String(Pair<Integer, Integer> tag) {
    ...
    // check if Key and Value are in cache
    if (this.tagToString.containsKey(keyInt)
        && this.tagToString.get(keyInt).getValue().containsKey(valInt)) {
      return new ImmutablePair<>(this.tagToString.get(keyInt).getKey(),
          this.tagToString.get(keyInt).getValue().get(valInt));
    }
  ...
  /**
   * Gets all values (ID and String) for a given key. To be used with care, as it forcibly queries
   * the DB each time it is called to be accurate. Preferably call only once per key you are
   * interested in.
   *
   * @param key The key to query all values for
   * @return a pair that holds the ID of the given key and all values with their respective string
   *         and integer, or null if it cannot be found
   */
  @Deprecated
  public Pair<Integer, Map<String, Integer>> getAllValues(String key) {
    ...
    if (keyid != null) {
      try (PreparedStatement valstmt = this.conn.prepareCall(
          "select VALUEID,TXT from " + TableNames.E_KEYVALUE.toString() + " where KEYID=?;")) {
  ...
  public Integer usertoID(String name) {
    ...
    try (PreparedStatement userstmt = conn
        .prepareStatement("select ID from " + TableNames.E_USER.toString() + " WHERE NAME = ?;")) {
  ...
  public String usertoStr(Integer uid) {
    ...
    String name = null;
    try (PreparedStatement Userstmt = conn
        .prepareStatement("select NAME from " + TableNames.E_USER.toString() + " WHERE ID = ?;")) {

...
/**
 * An object which allows to convert OSM tags (represented as strings) to and from the
 * internal representation of them in the form of {@link OSHDBTag} objects.
 *
 * <p>Note that it is only intended to use the `lookup*` methods on OSHDBTag objects
 * which had been generated by the same TagTranslator object before (or a different
 * TagTranslator object operating on the same keytables data set as the original one).
 * In all other cases, the resulting behaviour is undefined and might result in a
 * `null` value being returned, a run time exception being thrown, or some other
 * undefined behaviour.
 * </p>
 */

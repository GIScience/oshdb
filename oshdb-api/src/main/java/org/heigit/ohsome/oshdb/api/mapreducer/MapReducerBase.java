...
public abstract class MapReducerBase<X> implements
  ...
  public MapAggregator<OSHDBTimestamp, X> aggregateByTimestamp()
    ...
    return new Aggregator<>(this, indexer, this.getZerofillTimestamps());
  }
  ...
  public MapAggregator<OSHDBTimestamp, X> aggregateByTimestamp(
    ...
    return new Aggregator<>(this, (data, ignored) -> {
  ...
  public List<X> collect() {
    return this.reduce(
        ...
        MapReducerBase::collectCombiner
    );
  }
  ...
  private MapReducer<Number> makeNumeric() {
    return this.map(MapReducerBase::checkAndMapToNumeric);
  }
  ...
  private static class Aggregator<U extends Comparable<U> & Serializable, X>
    ...
    */
    private Aggregator(MapReducerBase<X> mapReducer, SerializableBiFunction<X, Object, U> indexer,
        Collection<U> zerofill) {
      this.mapReducer =
    ...
    private Aggregator(Aggregator<U, ?> obj,
    ...
    private <R> MapAggregator<U, R> copyTransform(MapReducerBase<IndexValuePair<U, R>> mapReducer) {
      return new Aggregator<>(this, mapReducer);
    }

    @Contract(pure = true)
    private <V extends Comparable<V> & Serializable> Aggregator<V, X> copyTransformKey(
        MapReducerBase<IndexValuePair<V, X>> mapReducer) {
      @SuppressWarnings("unchecked") // we convert the mapAggregator to a new key type "V"
      Aggregator<V, ?> transformedMapAggregator = (Aggregator<V, ?>) this;
      return new Aggregator<>(transformedMapAggregator, mapReducer);
    }
    ...
    @Contract(pure = true)
    private <V extends Comparable<V> & Serializable> Aggregator<V, X> mapIndex(
    ...
    private <A, B> SortedMap<U, B> transformSortedMap(SortedMap<U, A> in,
        Function<A, B> transform) {
      return in.entrySet().stream()
          .collect(Collectors.toMap(Entry::getKey, e -> transform.apply(e.getValue()), (v1, v2) -> {
            assert false;
            return v1;
          }, TreeMap::new));
    }
    ...
    private static class IndexValuePair<U, X> {
      ...
      public boolean equals(Object other) {
        return this == other || other instanceof IndexValuePair

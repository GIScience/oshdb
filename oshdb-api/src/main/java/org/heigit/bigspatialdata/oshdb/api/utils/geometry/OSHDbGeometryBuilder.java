...
public class OSHDbGeometryBuilder {
   ...
   * @return
   */
  public static <T extends OSMEntity> boolean isAuxiliary(T entity, Set<Integer> uninterestingTagKeys) {
  ...
  public static <T extends OSMEntity> Geometry getGeometry(T entity, long timestamp, TagInterpreter areaDecider) {
    if (entity instanceof OSMNode) {
      ...
      GeometryFactory geometryFactory = new GeometryFactory();
      Coordinate[] coords = way.getRefEntities(timestamp)
    ...
    for (int i = 0; i < relation.getMembers().length; i++) {
      try {
        geoms[i] = OSHDbGeometryBuilder.getGeometry(relation.getMembers()[i].getEntity().getByTimestamp(timestamp), timestamp, areaDecider);
      } catch (NullPointerException ex) {
        LOG.warn("Member entity of relation/{} missing, geometry could not be created.", entity.getId());
        return null;
      }
    }
  ...
  private static Geometry getMultiPolygonGeometry(OSMRelation entity, long timestamp, TagInterpreter tagInterpreter) {
    GeometryFactory geometryFactory = new GeometryFactory();

    Stream<OSMWay> outerMembers = entity.getMemberEntities(timestamp, tagInterpreter::isMultipolygonOuterMember)
            .map(osm -> (OSMWay) osm)
            .filter(way -> way != null && way.isVisible());

    Stream<OSMWay> innerMembers = entity.getMemberEntities(timestamp, tagInterpreter::isMultipolygonInnerMember)
            .map(osm -> (OSMWay) osm)
            .filter(way -> way != null && way.isVisible());
  ...
  public static <T extends OSMEntity> Geometry getGeometryClipped(T entity, long timestamp, TagInterpreter areaDecider, BoundingBox clipBbox) {
  ...
  public static <P extends Geometry & Polygonal, T extends OSMEntity> Geometry getGeometryClipped(T entity, long timestamp, TagInterpreter areaDecider, P clipPoly) {
    Geometry geom = OSHDbGeometryBuilder.getGeometry(entity, timestamp, areaDecider);
    if (geom == null) {
      return null;
    }

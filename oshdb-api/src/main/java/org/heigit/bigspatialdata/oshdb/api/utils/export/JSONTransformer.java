...
public class JSONTransformer {
   ...
   */
  public static <T extends OSMEntity> JsonObject transform(T entity, long timestamp, TagTranslator tagtranslator, TagInterpreter areaDecider) {
//JSON for properties
    ...
    properties.add("@id", entity.getId())
    ...
    jsonid.append("/").append(entity.getId()).append("@").append(TimestampFormatter.getInstance().isoDateTime(timestamp));
    ...
    for (int i = 0; i < entity.getTags().length; i += 2) {
      properties = JSONTransformer.addRiskyKey(entity, entity.getTags()[i], entity.getTags()[i + 1], tagtranslator, properties);
    }
    ...
    if (entity instanceof OSMWay) {
      ...
      for (OSMMember mem : relation.getMembers()) {
        ...
        try {
          ...
          LOG.warn("The TagTranslator could not resolve the member role {} of a member of relation/{}", mem.getRoleId(), entity.getId());
    ...
    try {
      ...
      result.add("type", "Feature").add("id", jsonid.toString()).add("properties", properties).add("geometry", geom);
    } catch (NullPointerException ex) {
      LOG.warn("Could not build the geometry of entity {} at timestamp {}", entity.toString(), TimestampFormatter.getInstance().isoDateTime(timestamp), ex);
      result.add("type", "Feature").add("id", jsonid.toString()).add("properties", properties).add("geometry", "<error: could not create geometry>");
    }

    return result.build();
  }

  private static <T extends OSMEntity> JsonObjectBuilder addRiskyKey(T entity, int key, int value, TagTranslator tagtranslator, JsonObjectBuilder properties) {
    ...
    try {
      ...
      try {
        LOG.warn("The TagTranslator could not resolve a value (ValueID: {}) of Entity {}", value, entity.toString(), ex);
        String tempkey = tagtranslator.key2String(key);
        return properties.add(tempkey, "<error: could not resolve value>");
      } catch (NullPointerException ex2) {
        LOG.debug("The TagTranslator could ALSO not resolve the key (KeyID: {}) of Entity {}", key, entity.toString(), ex2);
  ...
  /**
   * Convert multiple features at once. Calls multiTransform for each feature
   * but puts them in a FeatureCollection, so you can see them all at once in
   * your GIS.
   *
   ...
   * (https://tools.ietf.org/html/rfc7946#section-3.3
   */
  public static JsonObject multiTransform(List<Pair<? extends OSMEntity, Long>> osmObjects, TagTranslator tagtranslator, TagInterpreter areaDecider) {
    ...
    osmObjects.stream().forEach((Pair<? extends OSMEntity, Long> OSMObject) -> {
      aBuilder.add(JSONTransformer.transform(OSMObject.getKey(), OSMObject.getValue(), tagtranslator, areaDecider));
    });
  ...
  /**
   * Get a GIS-compatible version of your OSH-Object. Note that this method uses
   * the timestamps of the underlying OSM-Entities to construct geometries. If
   * you desire inter-version-geometries, which are possible, please refer to
   ...
   * (https://tools.ietf.org/html/rfc7946#section-3.3)
   */
  public static <T extends OSHEntity> JsonObject transform(T entity, TagTranslator tagtranslator, TagInterpreter areaDecider) {
  ...
  /**
   * Get a GIS-compatible version of your OSH-GridCell. Note that this method
   * uses only the latest version of the each underlying OSM-Entity to construct
   * geometries. If you desire inter-version-geometries, which are possible, or
   * specific geometries of all versions in this cell, please refer to
   * {@link #multiTransform(java.util.List, org.heigit.bigspatialdata.oshdb.util.TagTranslator, org.heigit.bigspatialdata.oshdb.util.tagInterpreter.TagInterpreter) }
   ...
   * (https://tools.ietf.org/html/rfc7946#section-3.3)
   */
  public static <T extends GridOSHEntity> JsonObject transform(T entity, TagTranslator tagtranslator, TagInterpreter areaDecider) {
    List<Pair<? extends OSMEntity, Long>> entities = new ArrayList<>(1);

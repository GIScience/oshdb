  ...
  public static <P extends Geometry & Polygonal> Stream<IterateAllEntry> iterateAll(
    ...
    for (OSHEntity<OSMEntity> oshEntity : (Iterable<OSHEntity<OSMEntity>>) cell) {
      ...
      osmEntityLoop: for (Map.Entry<Long, OSMEntity> entity : osmEntityByTimestamps.entrySet()) {
        ...
        // skip node versions outside of the current cell // todo: we can get rid of this with the
        // final db layout (nodes stored the same as other entity types)
        if (osmEntity instanceof OSMNode) {
          OSMNode node = (OSMNode) osmEntity;
          if (!node.isVisible()) {
            if (prev == null || prev.activities.contains(ContributionType.DELETION)) // previous
            // version was
            // deleted ->
            // skip
            {
          ...
          // todo: add case for node leaving bbox region -> (next version is probably not in out
          // list of cells anymore) -> issue "deletion" result here and continue
          if (cell.getId() != nodeGrid.getId(node.getLongitude(), node.getLatitude())) {

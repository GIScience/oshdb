package org.heigit.ohsome.oshdb.tool.importer.transform.reader;
...
public class TransfromNodeReaders implements Closeable {
  private static class TransformNodeReader extends TransformReader<TransformOSHNode> {
    ...
    @Override
    protected TransformOSHNode getInstance(byte[] data, int offset, int length, long baseId,
        long baseTimestamp, long baseLongitude, long baseLatitude) throws IOException {
      return TransformOSHNode.instance(data, offset, length, baseId, baseTimestamp, baseLongitude,
          baseLatitude);
    }
  }

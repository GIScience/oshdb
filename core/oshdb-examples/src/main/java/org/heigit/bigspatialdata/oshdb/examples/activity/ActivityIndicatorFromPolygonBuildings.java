...
public class ActivityIndicatorFromPolygonBuildings {
  ...
  /**
   * Computes an Activity Indicator for buildings inside a polygon.
   * This activity indicator counts all changes of building objects,which are geometry changes + tag changes (way versions) + tag and relation_member changes (relation versions)
   * @param conn connection object to OSHDB e.g h2
   * @param polygon to define analyis area
   * @param ti TagInterpreter to decide whether to build polygons or linestrings
   * @return Map<Long,Long> with timestamps and counts
   * @throws ClassNotFoundException
   * @throws ParseException
   * @throws IOException
   */
	public Map<Long,Long> execute(Connection conn, Geometry polygon, TagInterpreter ti) throws ClassNotFoundException, ParseException, IOException {
		...
		//create BBOX to query gridcells as CellId-Objects, which contain (zoomlevel,id)
    ...
    BoundingBox inputBbox = new BoundingBox(minLon, maxLon, minLat, maxLat);
		...
		//start processing in parallel all grid cells that relate to the input polygons BBOX 
			Map<Long,Long> superresult = cellIds.parallelStream().flatMap(cellId -> {
			  
				try (final PreparedStatement pstmt = conn
						.prepareStatement("(select data from grid_way where level = ?1 and id = ?2) union (select data from grid_relation where level = ?1 and id = ?2)")) {
					pstmt.setInt(1, cellId.getZoomLevel());
					pstmt.setLong(2, cellId.getId());

					//each time 2 gridCells (1 grid_way and 1 grid_relation) are queried and put into a stream
...
//						}
          ...
          OSMEntity osm = osh.getByTimestamp(t);
          try {
            if (osm.isVisible() && osm.hasTagKey(0) && osm.getGeometry(t, ti).isValid()
                && osm.getGeometry(t, ti).isSimple() && osm.getGeometry(t, ti).getCentroid().intersects(inputPolygon)) {

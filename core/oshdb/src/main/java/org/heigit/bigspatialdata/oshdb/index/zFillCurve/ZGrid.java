...
public class ZGrid {
	 ...
	 * <br><br>
	 * found here: https://gist.github.com/JLChnToZ/ec41b1b45987d0e1b40ceabc13920559
	 * @param 
	 * @return XY(x,y)
	 */
	...
	public static ArrayList<Integer> getNeighbours(int cellID) {
	  ...
	  public Set<Long> bbox2CellId(BoundingBox bbox, boolean enlarge) {
	    ...
	    return this.bbox2CellId( mdBbox, enlarge);
	  }
	   ...
	   */
	  public Set< Long> bbox2CellId(MultiDimensionalNumericData bbox, boolean enlarge) {
	    ...
	    if (minlong > maxlong) {
	      ...
	      result.addAll(bbox2CellId(new BasicNumericDataset(dataPerDimension), enlarge));
		...
		/**
		 * Helper method <br><br>
		 * checks if the given bbox size is passed and if so, splits up the given bbox into two new ones<br>
		 * if the bbox has the coordinates -91.0,190.0,-89.0,89.0, it will return one box with -91,00,180,00, -89,00,89,00
		 * and one with -180,00,-170,00, -89,00,89,00
		 * 
		 * <br><br>
		 * 
		 * @param 
		...
		private LinkedList<BoundingBox> bboxSplitter(BoundingBox bbox, boolean enlarge) throws ZGridException {
			...
			LinkedList<BoundingBox> collectBboxes = new LinkedList<BoundingBox>();
		...
		public Iterator<Long> bbox2CellIdIterator(BoundingBox bbox, boolean enlarge) throws ZGridException {
			...
			if (bbox2.minLon > 180.0) {
				...
				bbox = new BoundingBox(newMinLon, newMaxLon,bbox2.minLat, bbox2.maxLat);
			...
			LinkedList<BoundingBox> bboxCount = new LinkedList<BoundingBox>(bboxSplitter(bbox2, enlarge));        
			...
			if (bboxCount.size() > 1){	
				...
				while (iter.hasNext()){
					...
					BoundingBox bbox3 =  (BoundingBox) iter.next();
		...
		/**
		 ...
		 class ZGridException extends Exception {
			   public ZGridException(String msg){
			      super(msg);
			   }
		 }
